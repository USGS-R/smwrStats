...
#'
#'The \code{kensen} method for \code{plot} graphs the y and t data with
#'the best fit line.
#'
#'The \code{ppcc} method creates a single graph that shows the q-normal
#'plot with a line showing the fit.
#'
...
#' @param which either "All" or a number indicating which plot,
#'see \bold{Details}.
...
plot.htest <- function(x, which="All", set.up = TRUE, ...)
  ...
  ##    Note replicated from USGSwsStats
  NextMethod("plot")

#' @rdname plot.htest
#' @S3method plot kensen
#' @method plot kensen
