...
\leftskip=-0.5in
\rightskip=-0.5in
%\VignetteIndexEntry{Trends Example}
%\VignetteDepends{USGSwsStats}
%\VignetteDepends{USGSwsData}
...
These examples demonstrate some functions in the \texttt{USGSwsStats} package that facilitate trend analysis. The first example uses the ConecuhFlows dataset from Appendix C2 in Helsel and Hirsch (2002) to illustrate trend analysis of annual series data. The second example uses the KlamathTP data set in the USGSwsData package to illustrate seasonally adjusted trend analysis. This example uses the data modified for censoring in uncensored techniques. All section references in these examples are from Helsel and Hirsch (2002). The critical alpha value for all tests is 0.05.
...
Helsel and Hirsch (2002) stress plotting the data to help undserstand the data and find an appropriate statistical technique. For this analysis, a simple graph of the annual mean flow by year shows a downward trend. The \texttt{kensen.test} function is used to describe the trend.

<<echo=TRUE>>=
setSweave("trend01", 5, 5)
with(ConecuhFlows, timePlot(Year, Flow, Plot=list(what="points")))
# Required call to close PDF output graphics
graphics.off()
@
<<results=tex, echo=FALSE>>=
cat("\\includegraphics{trend01.pdf}\n")
cat("\\paragraph{}\n")
@
...
@
<<results=tex, echo=FALSE>>=
cat("\\includegraphics{trend02.pdf}\n")
cat("\\paragraph{}\n")
@
...
@
<<results=tex, echo=FALSE>>=
cat("\\includegraphics{trend03.pdf}\n")
cat("\\paragraph{}\n")
@
...
# The trend analysis
KTP.lm <- lm(log(TP_ss2) ~ Dectime + log(Flow) + fourier(Dectime),
						 data=KlamathTP)
summary(KTP.lm)
@

The attained p-value for the trend is about 0.0093, which is less than the critical alpha level set for the test. The value of the coefficient (-0.07646) can be used to estimate the trend in percent per year: 100 * (exp(-0.07646) - 1) = -7.36. That value, with the median concentration from the previous nonparamteric analysis can be used to estimate the trend in mg/L: (-7.36 / 100) * 0.075 = -0.0055, approximately.
