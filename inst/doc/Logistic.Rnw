...
This examples demonstrates the \texttt{binaryReg} and other logistic regression support functions in the \texttt{USGSwsStats} package. The example uses the PugetNitrate dataset from Tesoriero and Voss (1997). The dataset is available from the \texttt{USGSwsData} package.
