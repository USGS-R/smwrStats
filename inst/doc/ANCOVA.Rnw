...
\leftskip=-0.5in
\rightskip=-0.5in
%\VignetteIndexEntry{ANCOVA Example}
...
This examples demonstrates the \texttt{ancovaReg} function in the \texttt{USGSwsStats} package. The example uses the UraniumTDS dataset C16 from Helsel and Hirsch (2002). The dataset is available from the \texttt{USGSwsData} pacakge.
